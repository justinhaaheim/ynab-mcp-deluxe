...
1. `src/types.ts` - Added subtransaction input types
2. `src/server.ts` - Added schemas and updated tool descriptions
3. `src/ynab-client.ts` - Handle subtransactions in create/update logic

## Status

- [x] Research YNAB API subtransaction structure
- [x] Update types (`CreateSubTransactionInput`, `UpdateSubTransactionInput`)
- [x] Update Zod schemas (`SubTransactionInputSchema`)
- [x] Update ynab-client (`createTransactions`, `updateTransactions`)
...
- For `create_transactions`, subtransactions use selectors (category/payee by name or ID), matching the parent transaction pattern
- For `update_transactions`, subtransactions use raw IDs, matching the existing pattern for that tool
- Added TODO comment in `ynab-client.ts` about potentially supporting a merge mode for subtransaction updates
